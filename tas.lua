			...
			for i=0, #pico8.keymap[p] do
				for _, testkey in pairs(pico8.keymap[p][i]) do
					if love.keyboard.isDown(testkey) then
						curr_inputstate.keys = bit.bor(curr_inputstate.keys, 2^(i + p * 8))
						break
					end
				end
			end
...
function tas:draw_piano_roll()
	...
	if self.mouse_enabled then
		self.pianoroll_inputs[20] = "M" -- 20 should be more than any button
	end
	...
	for i=start_row, math.min(start_row + num_rows - 1, #self.inputstates) do
		...
		for j, _ in pairs(self.pianoroll_inputs) do
			if self.pianoroll_inputs[j] ~= "M" then
